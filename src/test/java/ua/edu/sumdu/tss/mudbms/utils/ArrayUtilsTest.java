...
class ArrayUtilsTest {
    ...
    void readAndWriteInt() {
        ...
        byte afterEndIdx = 1 + ImmediateDiskStorage.INT_SIZE * 2;
        ...
        ArrayUtils.writeInt(test, 1 + ImmediateDiskStorage.INT_SIZE, t2);
        ...
        Assertions.assertEquals(t2, ArrayUtils.readInt(test, 1 + ImmediateDiskStorage.INT_SIZE));
    }
    ...
    void readAndWriteBoolean() {
        ...
        int afterEndIdx = beforeStartIdx + ImmediateDiskStorage.BOOL_SIZE * array.length + 1;
        byte beforeStart = test[beforeStartIdx];
        byte afterEnd = test[afterEndIdx];
        for (int i = 0; i < array.length; i++) {
            array[i] = Math.random() > 0.5;
            ArrayUtils.writeBoolean(test, beforeStartIdx + 1 + ImmediateDiskStorage.BOOL_SIZE * i, array[i]);
        }
        Assertions.assertEquals(beforeStart, test[beforeStartIdx]);
        Assertions.assertEquals(afterEnd, test[afterEndIdx]);
        for (int i = 0; i < array.length; i++) {
            assertEquals(array[i], ArrayUtils.readBoolean(test, beforeStartIdx + 1 + ImmediateDiskStorage.BOOL_SIZE * i));
        }
    }
